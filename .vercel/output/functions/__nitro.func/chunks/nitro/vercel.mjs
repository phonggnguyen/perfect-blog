...
import { defineEventHandler, handleCacheHeaders, createEvent, useCookies, sendRedirect, assertMethod, readBody, setCookie, lazyEventHandler, eventHandler, createApp, createRouter } from 'h3';
...
const _zB0uKI = defineEventHandler(({ req, res, context }) => {
  ...
  if (!mainDomain.includes(hostname)) {
    const currentHost = process.env.VERCEL === "1" ? hostname.replace(`.keypress.blog`, "") : hostname.replace(`.localhost:3000`, "");
...
const _iKleKM = lazyEventHandler(() => {
  const ipxOptions = {
...
const handlers = [
  ...
  { route: '/_ipx/**', handler: _iKleKM, lazy: false, middleware: false, method: undefined },
...
export { useNitroApp as a, useRuntimeConfig as u, vercel as v };
